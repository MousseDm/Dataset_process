    ...
    for camera_id in camera_ids:
        ...
        for i, (rgb, depth) in enumerate(tqdm(zip(rgbs, depths), total=len(rgbs))):
            ...
            with h5py.File(os.path.join(color_dir, rgb), "r") as f:
                color = f["dataset"][:]
                Image.fromarray((np.clip(color, 0, 1) * 255).astype(np.uint8)).save(
                    os.path.join(subscene_dir, f"{frame_id:06d}_rawcolor.png"))
            ...
            if brightness_valid.size < 100:  # 避免有效像素太少
                print(f"[Warning] Too few valid pixels in frame {frame_id}, skipping tone mapping.")
                scale = 1.0
            else:
                brightness_percentile = np.percentile(brightness_valid, percentile)
                if brightness_percentile < eps:
                    scale = 1.0
                else:
                    scale = (
